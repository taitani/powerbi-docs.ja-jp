...
この例の*売上合計*のように単純な加法測度の場合、比較は絶対値ではなく相対値に基づきます。 そのため Touring Bikes の販売は、すべてのカテゴリの全体的な売上のよりも低いですが、既定では、ビジュアルで使われてデュアル軸の自転車のすべてのカテゴリと Touring Bikes のさまざまな国の売上の比率を比較できるようにします。  ビジュアルの下にあるトグルを切り替えると、同じ軸に 2 つの値が表示され、簡単に絶対値を比較できます (下の画像をご覧ください)。    
...
おそらくコンピューターでどのような*異なる*ことを意味します。 たとえば、米国とカナダでは売上が全体的に次のように分かれるとします。
...
前の例にあった*売上*のような加法測度の場合、2 つの軸と適切なスケーリングで相対値を簡単に比較できる縦棒/横棒グラフが使用されます。 縦棒ではフィルターが適用された値が示され、横棒では全体値が示されます (通常、縦棒軸が左に、横棒軸が右に表示されます)。 使用して、行を表示、*階段状*破線が付いている、スタイルがグレーで入力します。 前の例の場合は、列軸の最大値が 4 で、行軸の最大値は 20 です。 できるようになりますアメリカ合衆国およびカナダの間でのフィルター処理されたと全体的な値の相対値を比較しやすい。 
...
さまざまな縦棒と各縦棒の値を調べた後、最も大きな違いを与える値セットが選択されます。 理解しやすいように縦棒別にグループ化された上で出力され、最も大きな違いを与える値を含む列が一覧の最初に表示されます。 列あたり最大で 3 つの値が表示されますが、影響が大きい場合は、3 つ未満の値があった場合、またはいくつかの値が他よりもよりインパクトのある場合は以下に示す場合があります。 
