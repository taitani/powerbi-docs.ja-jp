...
**[図形のマップ]** のビジュアルは ESRI/TopoJSON マップをベースにしています。このマップの強みは、地理的マップ、座席配置、フロア プランなどユーザーが作成できるカスタム マップを使用できるという点です。 **[マップのシェイプ]** のこのプレビュー リリースでは、カスタム マップを使用する機能はサポートされていません。しかし、この機能がプレビューを終了すると、カスタム マップが有効になります。次の Power BI Desktop の月単位の更新がそのタイミングになると予想されます。
...
## <a name="use-custom-maps"></a>カスタムのマップの使用
...
* 現時点で、**[凡例]** の分類が適切に機能するには、**[値]** バケットも設定されている必要があります。 **[マップのシェイプ]** の最終リリースで、この動作を改善することを予定しています。
* **[マップのシェイプ]** のリリース バージョンには、現在選択されているマップのマップ キーを表示するユーザー インターフェイスが用意されます。このプレビューでは、この記事で後述する「**地域キー**」セクションに示した表のマップ地域キーを参照することができます。
