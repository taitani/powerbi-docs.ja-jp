...
グラフを並べ替えるには、グラフの右上にある省略記号 [...] を選び、 **[並べ替え]** を選んで、並べ替えるフィールドと向きを選びます。 詳しくは、「[Change how a visual is sorted](../consumer/end-user-change-sort.md)」 (ビジュアルの並べ替え方法を変更する) をご覧ください。
...
* 横棒/縦棒グラフは、ドリルダウンする (そして元に戻る) ことができます。  これは、余分なスペースを使わずに 1 つのビジュアルにより多くの情報を詰め込む優れた方法です。  次の例では、地域から国への階層が使われています。  地域のバーをダブルクリックすると、その地域を構成する国にドリルダウンします。  ドリルダウンの詳細については、「[Power BI での視覚化のドリルダウン](../consumer/end-user-drill.md)」をご覧ください。
