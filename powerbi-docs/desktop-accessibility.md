...
* フォーカスが読み込まれている*レポート ページ*にある場合、*Tab* キーを使用してページのすべてのテキスト ボックス、画像、図形、およびグラフなどの各オブジェクトにフォーカスを移動します。 スクリーン リーダーでは、オブジェクトの種類、オブジェクトのタイトル (ある場合)、およびレポート作成者によって指定されている場合はオブジェクトの説明が読み上げられます。 

ビジュアル間を移動するときは、**Alt + Shift + F10** キーを押して、ビジュアル ヘッダーにフォーカスを移動することができます。このヘッダーには、並べ替え、グラフの背後にあるデータのエクスポート、フォーカス モードなどのさまざまなオプションが含まれます。 
...
### <a name="table--matrix-navigation"></a>テーブルとマトリックスのナビゲーション
| 操作の内容           | 押すキー                |
| :------------------- | :------------------- |
| (すべての領域のすべてのセル) の間でフォーカスを 1 つのセルの上下に移動します。  | ↑ キーと下方向キー |
| フォーカスを左右に移動]、[(すべての領域のすべてのセル) の間で 1 つのセルを右  | 左方向キーまたは右方向キー |
