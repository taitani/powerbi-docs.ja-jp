...
アプリは定期的に新しい機能によって更新されます。 最新バージョンを実行していることを確認します。 アプリの以前のバージョンが既にインストールされている場合は、それをご利用のアプリの中から削除し、Ctrl + F5 キーを押して更新することをお勧めします。 
...
アプリをインストールしたので、組織で容量に関するメトリックを確認できるようになりました。 アプリでは、メトリックの概要のダッシュ ボードとレポートの詳細なメトリックを提供します。
...
| 更新の合計 | 過去 7 日間の更新の合計数です。 |
| 更新の信頼性 (%) | 過去 7 日間で成功した更新を更新の合計数で割った数です。 |
| CPU の最大値 (%)| 過去 7 日間に AI ワークロードの消費量を最大 CPU。 |
| メモリの最大値 (GB) | 最大メモリの消費量 AI ワークロードによって、過去 7 日間です。|
| 最大待機時間 (ミリ秒) を更新します。 | 更新を開始する前の最大時間。 |
| 更新の平均待機時間 (ミリ秒)| 更新が開始されるまでの平均時間です。 |
| 最大実行時間 (ミリ秒) を更新します。 | 更新を完了するの最大時間。 |
| 更新の平均実行時間 (ミリ秒)| 更新を完了するのにかかった平均時間です。 |
| | |
...
[**AI** ](#ai) -して容量で使用する AI 関数の正常性に関する詳細なメトリックを提供します。
...
| レポート セクション | メトリック |
| --- | --- |
| 更新 |  合計数: 各データセットの合計更新数です。<br>  信頼性:データセットごとに完了した更新の割合。<br>  Avg Wait Time\(平均待機時間\): データセットの更新のスケジュールされた時間と開始の間の平均ラグです (分)。<br>  Max Wait Time\(最大待機時間\): データセットの最大待機時間です (分)。<br>  Avg Duration\(平均実行時間\): データセットの更新の平均実行時間です (分)。<br>  Max Duration\(最大実行時間\): 最も時間のかかったデータセットの更新の実行時間です (分)。 |
...
| **レポート セクション** | **メトリック** |
| --- | --- |
| 更新 |  合計:各データフローの合計更新数です。<br>  信頼性:各データ フローの完了の更新の割合。<br>  Avg Wait Time\(平均待機時間\): データフローの更新のスケジュールされた時間と開始の間の平均ラグです (分)。<br>  Max Wait Time\(最大待機時間\): データフローの最大待機時間です (分)。<br>  Avg Duration\(平均実行時間\): データフローの更新の平均実行時間です (分)。<br>  Max Duration\(最大実行時間\): データフローの最も時間のかかった更新の実行時間です (分)。 |
...
| **レポート セクション** | **メトリック** |
| --- | --- |
| 全体の使用状況 | 合計数:ワークスペース、またはデータ フローでの AI 関数の数。 <br> 信頼性:更新完了の割合。<br> 平均待機時間。スケジュールされた時刻と (ミリ秒単位)、更新の開始の間の平均遅延。<br> Max Wait Time\(最大待機時間\): 最大待機時間 (ミリ秒)。<br> 平均継続時間:ミリ秒単位で、更新の平均期間です。<br> Max Duration\(最大実行時間\): 最も長い実行中の期間はミリ秒単位で更新します。<br> 平均入力サイズ:平均入力サイズ (バイト)、データ フローの更新によって実行される AI 関数。<br> 平均出力サイズ:平均出力サイズ (バイト)、データ フローの更新によって実行される AI 関数。 |
| 平均実行時間による上位 5 件の AI 関数 | 最長の平均で 5 つの関数は、期間 (ミリ秒) を更新します。 |
| 平均入力サイズで上位 5 件の AI 関数 | 最大規模の平均データで 5 つの関数は、バイト単位のサイズを入力します。 |
| 1 時間ごとの更新数とメモリ消費量 | 成功数、失敗数、およびメモリ消費量です (1 時間のバケットに分割、UTC 時間で報告)。 |
| 1 時間あたりの平均実行時間 | 1 時間のバケットに分割し、更新の平均実行時間は、UTC 時刻で報告されます。 |
| | |
...
1. **[アクセス制御 (IAM)]** をクリックしてから、閲覧者ロールに **Power BI Premium** アプリを追加します。 名前で、アプリを検索しない場合は、追加することも、クライアント識別子を使用して:`cb4dc29f-0bf4-402a-8b30-7511498ed654`します。
