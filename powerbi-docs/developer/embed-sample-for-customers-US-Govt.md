---
title: "米国政府機関顧客向けのアプリケーションに Power BI コンテンツを埋め込む"
...
---
# <a name="embed-a-power-bi-dashboard-tile-or-report-into-your-application-for-us-government"></a>Power BI のダッシュボード、タイル、レポートを米国政府機関向けアプリケーションに埋め込む
顧客向けのダッシュボード、タイル、またはレポートを、Power BI .NET SDK と Power BI JavaScript API を使って Web アプリに統合する (埋め込む) 方法を説明します。 通常、これは ISV のシナリオです。

パブリック クラウドとは異なり、米国政府機関向けクラウドには 3 つの異なるカテゴリ セクションがあります。
...
このチュートリアルを開始するには、**Power BI 米国政府機関向けアカウント**が必要です。 アカウント セットアップを持っていない場合は、[政府機関向け Power BI アカウントにサインアップする](../service-govus-signup.md)ことができます。
...
REST API の呼び出しを行うには、Azure AD にアプリケーションを登録する必要があります。 詳しくは、「[Azure AD アプリを登録して Power BI コンテンツを埋め込む](register-app.md)」をご覧ください。 政府機関の種類に応じて 3 つの異なるカテゴリ セクションがあるため、アプリケーションを登録する 3 つの異なる URL があります。
...
アプリケーションでは、Power BI REST API の呼び出しを行う前に、まず、Azure AD から**アクセス トークン**を取得する必要があります。 詳しくは、「[ユーザーを認証し、Power BI アプリ用の Azure AD アクセス トークンを取得する](get-azuread-access-token.md)」をご覧ください。 政府機関の種類に応じて 3 つの異なるカテゴリ セクションがあるため、アプリケーションのアクセス トークンを取得する 2 つの異なる URL があります。
