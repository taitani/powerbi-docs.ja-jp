---
title: Power BI で高密度線サンプリング
description: Power BI で高密度線サンプリング
...
---
# <a name="high-density-line-sampling-in-power-bi"></a>Power BI で高密度線サンプリング
**Power BI Desktop** の 2017 年 6 月リリースおよび **Power BI サービス**の更新以降、新しいサンプリング アルゴリズムが使用可能です。これにより、高密度データをサンプリングする視覚エフェクトが改善されます。 たとえば、小売店の売上結果から折れ線グラフを作成するとします。各店舗の売上金は毎年 1 万を超えています。 このような売上情報の折れ線グラフは、店舗ごとのデータからデータ (時間の経過と共に売上がどのように変化するかを示すために、そのデータのわかりやすい表記を選択) をサンプリングし、基になるデータを表します複数系列折れ線グラフを作成します。 これは、高密度データを視覚化する一般的な方法です。 Power BI Desktop では高密度データのサンプリングが改善されました。詳細については、この記事で説明します。
...
## <a name="how-high-density-line-sampling-works"></a>どのように高密度線サンプリングのしくみ
これまで、**Power BI** では、決定論的な方法で基になるデータ全体のサンプル データ ポイントのコレクションを選択していました。 たとえば、1 カレンダー年にわたる視覚エフェクトの高密度データの場合、350 のサンプル データ ポイントが視覚エフェクトで表示されます。各ポイントを選択すると、データ全体 (基になるデータの系列全体) が視覚エフェクトで表されます。 このしくみを理解するために、1 年間の株価をプロットして (つまり、1 つのデータ ポイントの 1 日) を折れ線グラフ ビジュアルの作成に 365 のデータ ポイントの選択を想像してください。
...
高密度のビジュアルの**Power BI**インテリジェントにデータを高解像度のチャンクにスライスし、各チャンクを表すための重要なポイントを選択します。 高解像度のデータ スライスの処理が具体的には、結果として得られるグラフを確実に調整されているでは、視覚的に高速化と対話性ですが、基になるデータ ポイントのすべてのレンダリングと区別できません。

### <a name="minimum-and-maximum-values-for-high-density-line-visuals"></a>最小値と高密度線の視覚エフェクトの最大値
...
### <a name="evaluating-representative-data-points-for-high-density-line-visuals"></a>高密度線の視覚エフェクトの代表的なデータ ポイントを評価します。
...
たとえば、株価に基づいてビジュアルを作成して、どちらを使用している 2 つの異なる株を比較する**High-Density サンプリング**します。 各系列の基になるデータには多数のデータ ポイントがあります (常時、株価をキャプチャしていると考えられます)。 高密度線サンプリング アルゴリズムでは、各系列に対して個別にビン分割を実行します。
...
既定では、新しいアルゴリズムは**有効**になっています。 この設定を変更するには、**書式** ウィンドウで、**全般**カードの下部には、表示というトグル スライダー **High-Density サンプリング**します。 これを無効にするは、 **[オフ]** にスライドします。
...
* 細分性を高めたことで、またビン分割プロセスが原因で、**ヒント**で値を表示できるのは、代表データをカーソルで揃えた場合のみとなります。 参照してください、*ツールヒントと高密度線サンプリング*の詳細については、この記事のセクション。
