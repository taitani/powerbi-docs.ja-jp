...
使うことができる機能は、ユーザーが "*作成者*" (マップを作成したユーザー) か "*使用者*" (誰かから ArcGIS マップを共有されたユーザー) かによって異なります。 コンシューマー ([読み取りビュー](end-user-reading-view.md)とも呼ばれる) として ArcGIS マップと対話する場合、以下の操作を使用できます。

* "*ビュー*" アクセス許可を持つ Premium コンシューマーである場合、[視覚化の作成に使用されるデータの確認](end-user-show-data.md)、[サブスクライブ](end-user-subscribe.md)、[フォーカス モードおよび全画面表示モード](end-user-focus.md)でのマップの表示、[関連コンテンツの表示](end-user-related.md)、"*レポート作成者*" によって設定された[フィルターとの対話](end-user-report-filter.md)、[レポートの共有](../service-share-reports.md)などを行うことができます。
