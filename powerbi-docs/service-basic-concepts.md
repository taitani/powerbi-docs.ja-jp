...
この記事では、読者が既に [Power BI サービスにサインアップ](service-self-service-signup-for-power-bi.md)し、[いくつかデータを追加](service-get-data.md)していることを想定しています。 まだデータがない場合、[Power BI のサンプル コンテンツ パック](sample-datasets.md#the-power-bi-samples-as-content-packs)をインストールできます。

Power BI サービスを開くと、 **_ダッシュボード_** が表示されます。 ダッシュボードは、Power BI サービスが Power BI Desktop と異なる点です。

![ブラウザーでの Power BI サービス](media/service-basic-concepts/completenewest.png)

Power BI サービスのユーザー インターフェイス (UI) の主な機能は次のとおりです。
...
これらの機能については後に詳しく説明しますが、まずいくつかの Power BI の概念を確認します。
...
ユーザーが[ **_アプリ ワークスペース_** ](service-collaborate-power-bi-workspace.md)に属している場合は、あるワークスペース メンバーが追加したデータセットを他のワークスペース メンバーも利用できます。
...
* 1 つのデータセットのデータを使用して作成できます。 (稀な例外として、Power BI Desktop では複数のデータセットを組み合わせて 1 つのレポートにし、それを Power BI にインポートできます)
...
### <a name="1-navigation-pane-left-navpane"></a>1.**ナビゲーション ウィンドウ** (左ナビゲーション ウィンドウ)
...
* このアイコンを使用して、ナビゲーションバーを展開したり折りたたんだりします。 ![ナビゲーション ウィンドウ アイコン](media/service-basic-concepts/expand-icon.png).
