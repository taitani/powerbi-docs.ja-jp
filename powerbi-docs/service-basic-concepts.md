...
この記事では、読者が既に [Power BI サービスにサインアップ](service-self-service-signup-for-power-bi.md)し、[いくつかデータを追加](service-get-data.md)していることを想定しています。 まだデータがない場合、[Power BI サンプル コンテンツ パック](sample-datasets.md#the-power-bi-samples-as-content-packs)をインストールしてください。

![ブラウザーの Power BI サービス ホーム画面](media/service-basic-concepts/power-bi-home-screen.png)

ブラウザーで Power BI サービスを開くと、次のような要素が表示されます。
...
ワークスペースは、Power BI のダッシュボード、レポート、ブック、データセットのコンテナーです。 2 種類のワークスペース、*マイ ワークスペース*と*アプリ ワークスペース*があります。 ところで、"*アプリ*" とは何でしょうか。 Power BI の "*アプリ*" とは、組織に主要な指標を提供するために作成されたダッシュボードとレポートの集まりです。 アプリは対話型ですが、編集することはできません。
