---
title: "Excel から Power BI ダッシュボードにタイルをピン留めする"
...
2. Power BI から[そのブックに接続します](service-excel-workbook-files.md)。
3. Power BI で、ブックはワークスペースの **[ブック]** タブに追加されます。  アイコン ![](media/service-dashboard-pin-tile-from-excel/pbi_workbookicon.png) はこれが Excel ブックであることを示し、黄色のアスタリスクはそれが新しいものであることを示します。
   
    Power BI でブックに加えた変更は保存されず、OneDrive for Business にある元のブックには影響を与えません。 Power BI 内でデータの並べ替え、フィルター処理、または値の変更を行った場合、これらの変更を保存またはピン留めすることはできません。 ブックを更新するには、鉛筆アイコンを選択してブックを Excel Online で開きます。 Excel Online でのブックへの変更に基づいてタイルが更新されるまで、数分かかることがあります。     
...
2. ピン ![](media/service-dashboard-pin-tile-from-excel/pbi_pintile_small.png) アイコンを選びます。 
