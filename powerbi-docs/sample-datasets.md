...
ms.locfileid: "61206624"
---
# <a name="what-sample-data-is-available-to-use-with-power-bi"></a>Power BI で使用できるサンプル データ
Power BI を初めて試してみたくてもデータがない場合や、  または、データセットがある可能性があります。 理解していない (まだ) Power BI のしくみ、ため、データセットが壊れる可能性があることを心配でしょうか。

でも大丈夫です。 obviEnce ([www.obvience.com](http://www.obvience.com/)) と Microsoft から、Power BI を使い慣れていないユーザーのためのサンプルが提供されています。  データは匿名化されており、金融、人材、販売など、さまざまな業種のものがあります。 また、オンライン ドキュメントは、作業を進めるにできるように、チュートリアルと例については、これらのサンプルを使用します。      

   ![入手可能なサンプル](media/sample-datasets/power-bi-samples.png)

これらのサンプルはいずれも、コンテンツ パック、個別の Excel ブック、.pbix ファイルなどの複数の形式で利用できます。 内容や入手方法がわからなくても心配することはありません。 この記事ではそれについて説明します。 各サンプルを作成しました、*ツアー*します。 ツアーは、サンプルのストーリーし、さまざまなシナリオについて説明する記事です。 マネージャーの質問に答える、競争力について詳しく調べる、レポートやダッシュボードを作成して共有する、ビジネスのシフトを説明するなど、さまざまなシナリオがあります。   

始める前に、サンプルの使用に関する法的なガイドラインを示します。 その後はサンプルを紹介し、それらを使用する方法について説明します。   
...
ブックとデータは、obviEnce、LLC のプロパティを業界のサンプル データを使用して Power BI の機能を例示するためだけに共有されています。
...
この業界サンプルは、会社の経営陣、製品、および顧客のために、CFO の主要なメトリックを分析します。 会社の収益性に影響を与える要因を調査することができます。
...
この業界サンプルは、新規採用者、アクティブな従業員、退職した従業員を分析することにより、会社の採用戦略について説明します。  データを探索することにより、自主退職の傾向と採用戦略の偏りを調べることができます。
...
この業界サンプルの分析、計画的なと、会社の IT 部門の実際のコストを比較します。 比較することで当年度における予算が適切に組まれたかどうかを把握し、当初の計画から大きく乖離している分野を調べることができます。 この例の会社では年間予算が組まれ、四半期ごとに最新予測 (LE) が出されて当該会計年度における IT 支出の変化を分析しています。
...
前述のように、サンプルはコンテンツ パック、Excel ブック、.pbix ファイルなどの複数の形式で提供されます。 これらの形式の各使用方法のコンテンツ パックで始まる説明します。

## <a name="the-power-bi-samples-as-content-packs"></a>コンテンツ パックの Power BI サンプル
コンテンツ パックは、Power BI 内から使用できる唯一のサンプル形式です。Power BI から出なくても、コンテンツ パックを探すことができます。 コンテンツ パックは、1 つまたは複数のダッシュ ボード、データセット、およびユーザーを作成し、Power BI サービスで使用できるレポートのバンドルでは基本的にします。 ユーザーは、コンテンツ パックを作成して同僚と共有します。 各 Power BI サンプルのコンテンツ パックには、データセット、レポート、およびダッシュボードが含まれます。  コンテンツ パックは Power BI Desktop では使うことができません。 コンテンツ パックについて詳しくは、「[Intro to content packs in Power BI](service-organizational-content-pack-introduction.md)」 (Power BI でのコンテンツ パックの概要) をご覧ください。
...
2. 下部の左上隅にある次のように選択します。**データを取得する**します。
...
2. ダウンロードしたファイルを保存します。 どこにファイルを保存するかによって違いがあります。
   ...
   * ![ローカル](media/sample-datasets/power-bi-local-file2.png)  **ローカル** - コンピューター上のローカル ドライブまたは組織内の別の場所にファイルを保存した場合、ファイルを Power BI にインポートできます。 実際には、ファイルはファイル全体が Power BI にインポートされていないために、ローカル ドライブに残ります。 動作は、新しいデータセット データを Power BI サイトで作成された、場合によっては、データ モデルは、データセットに読み込まれます。 ファイルのすべてのレポートの場合は、Power BI サイトの [レポート] に、それらのレポートが表示されます。
   * ![OneDrive Business](media/sample-datasets/power-bi-onedrive-file.png) **OneDrive - ビジネス**– OneDrive for Business があり、そこには Excel では、作業内容を保持する最適な場所で最も使用すると、OneDrive for Business の Power BI にサインインする同じアカウントでサインインする場合Power BI、または。CSV ファイルの同期をデータセット、レポート、および Power BI のダッシュ ボード。 Power BI と OneDrive はどちらもクラウドにあるため、Power BI は約 1 時間ごとに OneDrive 上のファイルに接続します。 なんらかの変更が見つかった場合には、データセット、レポート、およびダッシュボードが Power BI で自動的に更新されます。
...
1. Power BI サービス (app.powerbi.com) を開き、サインインします。
   ...
   >
   >
4. 下部の左上隅にある次のように選択します。**データを取得する**します。
...
7. この場合、ファイルを選択**Analysis Sample.xlsx**、選択され、OneDrive for Business に保存されて**Connect**します。
...
10. **[データセットの準備ができました]** 選択画面が表示されたら、**データセットの表示**または**クイック分析情報の取得**か、Power BI 左側のナビゲーション バーを使用して検索し、関連付けられているレポートまたはダッシュ ボードを開きます。  
...
* 最初にサンプル Excel ブックを開くと、2 つの警告が表示されることがあります。 最初の警告は、ブックが保護されたビューで述べています。 **[編集を有効にする]** を選択してください。 第 2 の警告は、ブックに外部データ接続と可能性があります。 **[コンテンツの有効化]** を選択してください。
* 各ブックには、いくつかの Power View シートが含まれています。 Excel で Power View シートを表示する場合は、する必要があります[Power View アドインを有効にする](https://support.office.com/article/flash-silverlight-and-shockwave-controls-blocked-in-microsoft-office-55738f12-a01d-420e-a533-7cef1ff6aeb1)レジストリ キーのパッケージをダウンロードします。
* 実際のデータは、 Power Pivot データ モデルに存在しています。 データを表示する Power View シートは不要です。 **PowerPivot** ] タブで [**管理**します。

    表示されない、 **Power Pivot**タブですか? [Power Pivot のアドインを有効にします](https://support.office.com/article/Start-Power-Pivot-in-Microsoft-Excel-2013-add-in-A891A66D-36E3-43FC-81E8-FC4798F39EA8)。

    Power Pivot では、基になるすべてのテーブルとすべての DAX の数式でデータを確認できます。 
